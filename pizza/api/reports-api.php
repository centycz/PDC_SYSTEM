...
// NOVÁ FUNKCE - přidává payment filter bez LEFT JOIN duplicit
...
function getSalesData($params = []) {
    ...
    try {
        ...
        if ($view === 'default') {
            ...
            // OPRAVENO - používáme novou funkci pro payment filter
...
if ($view === 'top_orders') {
    $sql_base = "
        ...
        JOIN table_sessions ts ON o.table_session_id = ts.id
        WHERE DATE(oi.paid_at) IN ($placeholders)
        AND oi.status = 'paid'
    ";
    ...
    $sql = $sql_base . "
        GROUP BY oi.order_id, oi.payment_method, oi.paid_at, ts.table_number, o.employee_name
        ORDER BY amount DESC
        LIMIT 15
    ";
    
    // TOTO CHYBĚLO - vykonání dotazu a uložení výsledků
    $stmt = $pdo->prepare($sql);
    $stmt->execute($top_params);
    $top_orders = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $result['top_orders'] = $top_orders;
    
    error_log("Top orders count: " . count($top_orders));
}
        ...
        if ($view === 'analytics_data') {
            ...
            // Data pro graf platebních metod - zde pracujeme přímo s payments
            $sql = "
    SELECT 
        oi.payment_method,
        SUM(oi.unit_price * oi.quantity) as amount
...
";
            ...
            // OPRAVENO - přidáme payment filter i pro zaměstnance
        ...
        if (in_array($view, ['default', 'analytics', 'analytics_data'])) {
            ...
            // OPRAVENO
            list($sql_foodcost, $foodcost_params) = addPaymentFilter($sql_foodcost, $foodcost_params, $payment_method);
